...
<table class="layui-table" lay-data="{url:'${ctx}/memberCard/queryJson', page:true, id:'idTest',limit:10,method:'post'}" lay-filter="demo">
  <thead>
    <tr>
      ...
      <th lay-data="{field:'consumptionPoint',align:'center', width:160}">积分比例（元/分）</th>
      <th lay-data="{field:'enableVipprice',align:'center', width:100,templet:'#enableVippriceTemp'}">启用会员价</th>
...
<script type="text/html" id="enableVippriceTemp">
  {{# if(d.enableVipprice===1){ }}
 	 <span >是</span>
  {{# } }}
  {{# if(d.enableVipprice===0){ }}
 	 <span  >否</span>
  {{# } }}
</script>
<script type="text/html" id="barDemo">
  <a class="layui-btn layui-btn-mini" lay-event="edit">编辑</a>
  <a class="layui-btn layui-btn-mini" lay-event="selectItem">免费项目/产品</a>
</script>
...
layui.use(['laypage', 'table','layer','form'], function(){
  ...
  table.on('tool(demo)', function(obj){
    ...
    if(obj.event === 'detail'){
      ...
      layer.confirm('确定要删除【'+data.name+'】数据吗？', function(index){
   	 ...
   	 layer.open({
            ...
            ,shade: 0.8
            ,maxmin: true
            ,content: '${ctx}/memberCard/edit?dbid='+data.dbid
    ...
    else if(obj.event === 'selectProduct'){
   	 layer.open({
            type: 2 //此处以iframe举例
            ,title: '设置免费产品'
            ,area: ['760px', '560px']
            ,shade: 0.8
            ,maxmin: true
            ,content: '${ctx}/memberCard/selectProduct?member_card_id='+data.dbid
            ,zIndex: layer.zIndex //重点1
            ,success: function(layero){
              //layer.setTop(layero); //重点2
            }
   	 ...
   	 layer.open({
            ...
            ,shade: 0.8
            ,maxmin: true
            ,content: '${ctx}/memberCard/selectItem?member_card_id='+data.dbid
            ,zIndex: layer.zIndex //重点1
            ,success: function(layero){
              //layer.setTop(layero); //重点2
            }
