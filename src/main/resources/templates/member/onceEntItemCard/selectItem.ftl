...
<title>选择次卡可用项目</title>
</head>
<body>
	<form class="layui-form" id="frmId" name="frmId"   target="_parent" style="padding: 12px;">

		 <input type="hidden" name="countCardId" id="countCardId" value="${countCardId }">
			  ...
			  <div class="layui-form-item">
			    ...
			    <c:forEach var="item" items="${items }">
			    	<c:if test="${item.state>0 }">
				       <input type="checkbox" name="itemId" value="${item.itemId }" lay-skin="primary" title="${item.itemName }" checked="checked">
			    	</c:if>
			    	<c:if test="${item.state<0 }">
				       <input type="checkbox" name="itemId" value="${item.itemId }" lay-skin="primary" title="${item.itemName }">
			    	</c:if>
			    </c:forEach>
			    </div>
			   </div>
...
<script type="text/javascript">
	layui.use(['form', 'layedit', 'laydate','upload'], function(){
	  ...
	  form.on('submit(submitButton)', function(data){
		  ...
		  $.ajax({	
				url : '${ctx}/onceEntItemCard/saveSelectItem',
				data : params, 
				async : false, 
				timeout : 20000, 
				dataType : "json",
