...
<div class="location">
	...
	<a href="javascript:void(-1);" onclick="">次卡购买记录</a>
</div>
<br>
<div class="layui-row" >
...
<table class="layui-table" lay-data="{url:'${ctx}/stormMoneyOnceEntItemCard/queryJson', page:true, id:'idTest',limit:10,method:'post'}" lay-filter="demo">
  <thead>
    <tr >
      <th lay-data="{checkbox:true, fixed: true}"></th>
      <th lay-data="{field:'memberName',align:'center', width:90}">会员</th>
      <th lay-data="{field:'onceEntItemCardName',align:'center', width:100}">套餐名称</th>
      <th lay-data="{field:'itemName',align:'center', width:100}">项目</th>
      <th lay-data="{field:'money',align:'center', width:100,style:'color:red;'}">价格</th>
      <th lay-data="{field:'num',align:'center', width:80,style:'color:red;'}">次数</th>
      <th lay-data="{field:'remainder',align:'center', width:90,style:'color:red;'}">剩余次数</th>
      <th lay-data="{field:'orderNo',align:'center',height:60, width:140}">订单号</th>
      <th lay-data="{field:'createDate',align:'center', width:160}">购买时间</th>
      <th lay-data="{field:'stateName',align:'center', width:120}">状态</th>
      <th lay-data="{field:'cashierName',align:'center', width:100}">操作人</th>
      <th lay-data="{fixed: 'right', width:120, align:'center', toolbar: '#barOperate'}"></th>
    </tr>
  </thead>
</table>
...
layui.use(['laypage', 'table','layer','form','laydate'], function(){
    ...
    var data = obj.data;
	 if(obj.event === 'view'){
    	layer.open({
            ...
            ,shade: 0.8
            ,maxmin: true
            ,content: '${ctx}/stormMoneyOnceEntItemCard/view?stormMoneyOnceEntItemCardId='+data.dbid+'&memberId='+data.memberId
            ,zIndex: layer.zIndex //重点1
            ,success: function(layero){
            }
          });
    }else if(obj.event==='cancel'){
    	layer.confirm('您确定撤销会员次卡购买记录吗？',{icon: 5}, function(index){
    		$.post('/stormMoneyOnceEntItemCard/cancel?id='+data.dbid,{},function(data){
  ...
  var $ = layui.$, active = {
    exportExcel:function(){
    	window.location.href='${ctx}/stormMoneyOnceEntItemCard/exportExcel';
    }
  };
