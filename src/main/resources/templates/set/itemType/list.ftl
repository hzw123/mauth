...
<div class="location">
	...
	<a href="javascript:void(-1);" onclick="">项目类型</a>
</div>
...
<table class="layui-table" lay-data="{url:'${ctx}/itemType/queryJson', page:true, id:'idTest',limit:10,method:'post'}" lay-filter="demo">
  <thead>
    <tr>
      <th lay-data="{checkbox:true, fixed: true}"></th>
      <th lay-data="{field:'classify',align:'center', width:140,templet:'#classifyTemplate'}">分类</th>
      <th lay-data="{field:'name',align:'center', width:180}">名称</th>
      <th lay-data="{field:'orderNum',align:'center', width:120}">排序</th>
      <th lay-data="{field:'createTime',align:'center', width:180}">创建时间</th>
  ...
  table.on('tool(demo)', function(obj){
    ...
    if(obj.event === 'detail'){
      ...
      layer.confirm('确定要删除【'+data.name+'】数据吗？', function(index){
    	deleteById('${ctx}/itemType/delete',data.dbid);
        obj.del();
        layer.close(index);
      });
   	 ...
   	 layer.open({
	        ...
	        ,area: ['420px', '290px']
	        ,shade: 0.8
            ,content: '${ctx}/itemType/edit?dbid='+data.dbid
  ...
  var $ = layui.$, active = {
    getCheckData: function(){ //获取选中数据
      ...
      layer.confirm('确定要删除选择【'+data.length+'】条数据吗？', function(index){
	      ...
	      deleteById('${ctx}/itemType/delete',array.toString());
      });
    }
