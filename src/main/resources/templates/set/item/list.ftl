...
<table class="layui-table" lay-data="{url:'${ctx}/item/queryJson', page:true, id:'idTest',limit:10,method:'post'}" lay-filter="demo">
  <thead>
    <tr>
      <th lay-data="{checkbox:true, fixed: true}"></th>
      <th lay-data="{field:'no',align:'center', width:120}">编号</th>
      <th lay-data="{field:'name',align:'center', width:180}">名称</th>
      <th lay-data="{field:'itemType',align:'center', width:120,templet:'<div>{{d.itemType.name}}</div>'}">类型</th>
  ...
  table.on('tool(demo)', function(obj){
    ...
    if(obj.event === 'detail'){
      ...
      layer.confirm('确定要删除【'+data.name+'】数据吗？', function(index){
    	deleteById('${ctx}/item/delete',data.dbid);
        obj.del();
        layer.close(index);
      });
    }
        ...
        layer.confirm(mess, function(index){
        	stopOrStart('${ctx}/item/stopOrStart',data.dbid);
            layer.close(index);
          });
     }
   	 ...
   	 layer.open({
            ...
            ,shade: 0.8
            ,maxmin: true
            ,content: '${ctx}/item/edit?dbid='+data.dbid
  ...
  var $ = layui.$, active = {
    getCheckData: function(){ //获取选中数据
      ...
      layer.confirm('确定要删除选择【'+data.length+'】条数据吗？', function(index){
	      ...
	      deleteById('${ctx}/item/delete',array.toString());
      });
    }
