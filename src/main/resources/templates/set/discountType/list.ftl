...
<div class="location">
	...
	<a href="javascript:void(-1);" onclick="">折扣类型</a>
</div>
...
<table class="layui-table" lay-data="{url:'${ctx}/discountType/queryJson', page:true, id:'idTest',limit:10,method:'post'}" lay-filter="demo">
  <thead>
    <tr>
      <th lay-data="{checkbox:true, fixed: true}"></th>
      <th lay-data="{field:'name',align:'center', width:400}">名称</th>
      <th lay-data="{field:'type',align:'center', width:100,templet:'#sysTypeTemp'}">类型</th>
      <th lay-data="{field:'roleType',align:'center', width:120,templet:'#roleType'}">权限类型</th>
  ...
  table.on('tool(demo)', function(obj){
    ...
    if(obj.event === 'detail'){
      ...
      layer.confirm('确定要删除【'+data.name+'】数据吗？', function(index){
    	deleteById('${ctx}/discountType/delete',data.dbid);
        obj.del();
        layer.close(index);
      });
   	 ...
   	 layer.open({
            ...
            ,shade: 0.8
            ,maxmin: true
            ,content: '${ctx}/discountType/edit?dbid='+data.dbid
  ...
  var $ = layui.$, active = {
    getCheckData: function(){ //获取选中数据
      ...
      layer.confirm('确定要删除选择【'+data.length+'】条数据吗？', function(index){
	      ...
	      deleteById('${ctx}/discountType/delete',array.toString());
      });
    }
