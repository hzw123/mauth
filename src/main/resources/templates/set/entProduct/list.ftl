...
<div class="location">
	...
	<a href="javascript:void(-1);" onclick="">产品管理</a>
</div>
...
<div class="layui-row" >
    <div class="layui-col-xs6" >
	      <div class="demoTable" id="layerDemo" style="margin-left: 12px;">
			  <button class="layui-btn" onclick="window.location.href='${ctx}/entProduct/queryItemList'">选择产品</button>
    ...
    <div class="layui-col-xs6" >
      <form class="layui-form layui-input-inline" id="searchFrm" lay-filter="layform" method="post" style="float: right;" >
		  ...
		  <div class="layui-inline" style="width: 100px;">
		   <select id="productCategoryId" name="productCategoryId" class="layui-input" lay-filter="aihao" lay-verify="required">
				<option value="0">请选择...</option>
				<c:forEach items="${productCategories }" var="itemType">
...
<table class="layui-table" lay-data="{url:'${ctx}/entProduct/queryJson', page:true, id:'idTest',limit:10,method:'post'}" lay-filter="demo">
  <thead>
    <tr>
      <th lay-data="{checkbox:true, fixed: true}"></th>
      <th lay-data="{field:'no',align:'center', width:120,templet:'<div>{{d.product.sn}}</div>'}">编号</th>
      <th lay-data="{field:'name',align:'center', width:240,templet:'<div>{{d.product.name}}</div>'}">名称</th>
      <th lay-data="{field:'itemType',align:'center', width:120,templet:'<div>{{d.product.productcategory.name}}</div>'}">类型</th>
      <th lay-data="{field:'price',align:'center', width:100}">价格</th>
      <th lay-data="{field:'commissionNum',align:'center', width:100}">提成</th>
...
layui.use(['laypage', 'table','layer','form'], function(){
  ...
  table.on('tool(demo)', function(obj){
    ...
    if(obj.event === 'detail'){
      ...
      layer.confirm('确定要删除【'+data.product.name+'】数据吗？', function(index){
    	deleteById('${ctx}/entProduct/delete',data.dbid);
        obj.del();
        layer.close(index);
      });
    }
    	...
    	var mess="";
    	if(stopStatus==1){
    		mess='确定要下架【'+data.product.name+'】产品吗';
    	}
    	if(stopStatus==2){
    		mess='确定要上架【'+data.product.name+'】产品吗';
    	}
    	
        layer.confirm(mess, function(index){
        	stopOrStart('${ctx}/entProduct/stopOrStart',data.dbid);
            layer.close(index);
          });
     }
   	 ...
   	 layer.open({
            ...
            ,shade: 0.8
            ,maxmin: true
            ,content: '${ctx}/entProduct/edit?dbid='+data.dbid
  ...
  var $ = layui.$, active = {
    getCheckData: function(){ //获取选中数据
      ...
      layer.confirm('确定要删除选择【'+data.length+'】条数据吗？', function(index){
	      ...
	      deleteById('${ctx}/entProduct/delete',array.toString());
      });
    }
  ...
  form.on('submit(Search)', function (data) {
  	table.reload('idTest', {
  		where: {
  			...
  			no: data.field.no,
  			itemTypeId: data.field.productCategoryId
  		}
  	});
      return false;
  });
			...
			if (data[0].mark == 0) {// 删除数据成功提示信息
				...
				table.reload('idTest', {
			  		where: {
			  			name: $("#name").val(),
			  			no:  $("#no").val(),
			  			itemTypeId: $("#productCategoryId").val()
			  		}
			  	});
			}
		}
