...
<div class="location">
	...
	<a href="javascript:void(-1);" onclick="window.location.href='${ctx}/entProduct/queryList'">产品管理</a>-
	<a href="javascript:void(-1);" onclick="">选择产品</a>
</div>
<hr class="layui-bg-red">
<br>
<div class="layui-row" >
    	<form name="searchPageForm" id="searchPageForm" action="${ctx}/entProduct/queryItemList" method="post">
      	  产品类型：
		  <div class="layui-inline" style="width: 100px;">
		   <select id="productCategoryId" name="productCategoryId" class="layui-input" lay-filter="aihao" lay-verify="required" onchange="$('#searchPageForm')[0].submit()">
				<option value="0">请选择...</option>
				<c:forEach items="${productCategories }" var="itemType">
					<option value="${itemType.dbid }" ${param.productCategoryId==itemType.dbid?'selected="selected"':'' } >${itemType.name }</option>
				</c:forEach>
			</select>
		  </div>
		  编号：
...
var layer;
	...
	//时间选择器
	  ...
	  form.on('submit(submitButton)', function(data){
		  submitFrm('${ctx}/entProduct/saveItems');
		  return false
	  });
	});
</script>
