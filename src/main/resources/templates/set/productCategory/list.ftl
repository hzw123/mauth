...
<div class="location">
	...
	<a href="javascript:void(-1);" onclick="">产品类型</a>
</div>
...
<table class="layui-table" lay-data="{url:'${ctx}/productCategory/queryJson', page:true, id:'idTest',limit:10,method:'post'}" lay-filter="demo">
  <thead>
    <tr>
      <th lay-data="{checkbox:true, fixed: true}"></th>
      <th lay-data="{field:'name',align:'center', width:540}">名称</th>
  ...
  table.on('tool(demo)', function(obj){
    ...
    if(obj.event === 'detail'){
      ...
      layer.confirm('确定要删除【'+data.name+'】数据吗？', function(index){
    	deleteById('${ctx}/productCategory/delete',data.dbid);
        obj.del();
        layer.close(index);
      });
    }
   	 ...
   	 layer.open({
            ...
            ,shade: 0.8
            ,maxmin: true
            ,content: '${ctx}/productCategory/edit?dbid='+data.dbid
  ...
  var $ = layui.$, active = {
    getCheckData: function(){ //获取选中数据
      ...
      layer.confirm('确定要删除选择【'+data.length+'】条数据吗？', function(index){
	      deleteById('${ctx}/productCategory/delete',array.toString());
      });
    }
    ...
    setTop: function(){
        ...
        layer.open({
          ...
          ,shade: 0.8
          ,maxmin: true
          ,content: '${ctx}/productCategory/edit'
          ,zIndex: layer.zIndex //重点1
          ,success: function(layero){
            //layer.setTop(layero); //重点2
          }
