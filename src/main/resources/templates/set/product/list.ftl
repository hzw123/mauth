    ...
    <div class="layui-col-xs6" >
      <form class="layui-form layui-input-inline" id="searchFrm" lay-filter="layform" method="post" style="float: right;" >
		  ...
		  <div class="layui-inline" style="width: 100px;">
		   <select id="categoryId" name="categoryId" class="layui-input" lay-filter="aihao" lay-verify="required">
				<option value="0">请选择...</option>
				<c:forEach items="${productCategories }" var="productType">
					<option value="${productType.dbid }" >${productType.name }</option>
				</c:forEach>
			</select>
		  </div>
		  名称：
...
<table class="layui-table" lay-data="{url:'${ctx}/product/queryJson', page:true, id:'idTest',limit:10,method:'post'}" lay-filter="demo">
  <thead>
    <tr>
      <th lay-data="{checkbox:true, fixed: true}"></th>
      <th lay-data="{field:'sn',align:'center', width:120}">编号</th>
      <th lay-data="{field:'name',align:'center', width:180}">名称</th>
      <th lay-data="{field:'productType',align:'center', width:120,templet:'<div>{{d.productcategory.name}}</div>'}">类型</th>
      <th lay-data="{field:'price',align:'center', width:100}">价格</th>
      <th lay-data="{field:'commissionNum',align:'center', width:100}">提成</th>
      <th lay-data="{field:'unit',align:'center', width:100}">单位</th>
  ...
  table.on('tool(demo)', function(obj){
    ...
    if(obj.event === 'detail'){
      ...
      layer.confirm('确定要删除【'+data.name+'】数据吗？', function(index){
    	deleteById('${ctx}/product/delete',data.dbid);
        obj.del();
        layer.close(index);
      });
    }
        ...
        layer.confirm(mess, function(index){
        	stopOrStart('${ctx}/product/stopOrStart',data.dbid);
            layer.close(index);
          });
     }
   	 ...
   	 layer.open({
            ...
            ,shade: 0.8
            ,maxmin: true
            ,content: '${ctx}/product/edit?dbid='+data.dbid
  ...
  var $ = layui.$, active = {
    getCheckData: function(){ //获取选中数据
      ...
      layer.confirm('确定要删除选择【'+data.length+'】条数据吗？', function(index){
	      ...
	      deleteById('${ctx}/product/delete',array.toString());
      });
    }
